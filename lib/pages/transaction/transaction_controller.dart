...
import 'package:emprestapro/pages/transaction/transaction_state.dart';
...
class TransactionController extends ChangeNotifier {
  ...
  Future<void> update({
    ...
    final result =
        await transactionRepository.insert(transactionModel: transactionModel);

    result.fold(
      (error) => _changeState(TransactionErrorState(message: error.message)),
      (transaction) {
        homeController.transactons.add(transactionModel);
        _changeState(TransactionSuccessState());
      },
    );
  }
